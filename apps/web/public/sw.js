...
(() => {
  ...
  function isDocumentsApiRequest(url) {
    return /\/api\/t\/[^/]+\/documents(\/|$)/.test(url.pathname);
  }
  function isStaticAssetRequest(url) {
    return url.pathname.startsWith("/_next/static/") || url.pathname.startsWith("/icons/") || url.pathname.endsWith(".woff2");
  }
  ...
  self.addEventListener("activate", (event) => {
    event.waitUntil(
      caches.keys().then(
        (keys) => Promise.all(
          keys.filter((key) => ![SHELL_CACHE, NEWS_CACHE, DOCUMENTS_CACHE].includes(key)).map((key) => caches.delete(key))
        )
